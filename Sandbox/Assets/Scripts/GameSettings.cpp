...
GameSettings::Category GameSettings::GetCategory(int32 setting)
{
	if (setting <= 5) return Category::Graphics;
	if (setting <= 7) return Category::Sound;
	if (setting <= 10) return Category::Accessibility;
	return Category::Controls;
}

const char* GameSettings::CategoryName(Category category)
{
	switch (category)
	{
		case Category::Graphics: return "GRAPHICS";
		case Category::Sound: return "SOUND";
		case Category::Accessibility: return "ACCESSIBILITY";
