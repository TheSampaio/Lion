...
| `Sandbox/` | `Game` | Game | `lion-game.dll` | The game's code, as a module. Its assets live in `Sandbox/Assets/`. |
...
MSBuild Lion.sln -t:Game\Game -p:Configuration=Debug -p:Platform=x64  # only the game module
```
...
groups renames its target** — the editor's Compile builds `Game\Game` and would break silently.
