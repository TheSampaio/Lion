...
A 2D C++ game engine. `Lion.dll` is the engine; `Game.dll` is the game, loaded at runtime by both the
thin `Sandbox.exe` launcher and the editor; `Editor.exe` is the tooling.
...
MSBuild Lion.sln -t:Misc\Game -p:Configuration=Debug -p:Platform=x64   # only the game module
```

Verify against **Debug and Shipping** — Shipping strips logging and changes the entry point, so it
catches things Debug does not.

Run the editor from `.Out/Bin/Debug/Editor/` and the game from `.Out/Bin/Debug/Sandbox/`; both resolve
